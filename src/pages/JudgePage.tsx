...
import { getAllSubmissions, saveEvaluation, getEvaluationsForSubmission, isJudge } from '@/lib/firestore';
import { Submission, Evaluation, EvaluationScores } from '@/types';
import { Loader2, Shield, Star, Send, ChevronDown, ChevronUp, LogOut, CheckCircle } from 'lucide-react';
...
const defaultScores = (): EvaluationScores => ({ innovation: 5, feasibility: 5, impact: 5, clarity: 5 });
...
export default function JudgePage() {
  ...
  useEffect(() => {
    ...
    const check = async () => {
      ...
      await Promise.all(submitted.map(async (sub) => {
      ...
      submitted.forEach((sub) => {
        ...
        initialForms[sub.uid] = { scores: existing?.scores ?? defaultScores(), remarks: existing?.remarks ?? '', saving: false, saved: !!existing };
      });
  ...
  const updateScore = (uid: string, key: keyof EvaluationScores, val: number) => {
    setForms((prev) => ({ ...prev, [uid]: { ...prev[uid], scores: { ...prev[uid].scores, [key]: val }, saved: false } }));
  };

  const updateRemarks = (uid: string, val: string) => {
    setForms((prev) => ({ ...prev, [uid]: { ...prev[uid], remarks: val, saved: false } }));
  };
  ...
  const handleSaveEvaluation = async (sub: Submission) => {
    ...
    await saveEvaluation({ submissionId: sub.uid, judgeId: user.uid, judgeName: user.displayName || '', scores: form.scores, total, remarks: form.remarks });
    setForms((prev) => ({ ...prev, [sub.uid]: { ...prev[sub.uid], saving: false, saved: true } }));
  };
    ...
    return (
      <div className="min-h-screen bg-mesh flex items-center justify-center p-4">
        <div className="glass-card p-10 text-center max-w-sm">
          ...
          <p className="text-gray-400 text-sm mb-6">You are not registered as a judge. Contact the admin.</p>
      ...
      <nav className="border-b border-white/5 bg-black/30 backdrop-blur-md sticky top-0 z-10">
        <div className="max-w-5xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={signOut} className="text-gray-400 hover:text-white transition-colors p-2"><LogOut className="w-4 h-4" /></button>
          </div>
        </div>
      </nav>
            ...
            {submissions.map((sub, idx) => {
              ...
              return (
                <div key={sub.uid} className={`glass-card overflow-hidden transition-all duration-300 ${form.saved ? 'border-green-500/20' : ''}`}>
                  <button id={`judge-toggle-${idx}`} onClick={() => setExpanded(isOpen ? null : sub.uid)} className="w-full p-5 flex items-center justify-between gap-4 text-left hover:bg-white/5 transition-colors">
                    <div className="flex items-center gap-4">
                      <div className="w-10 h-10 rounded-full bg-gradient-to-br from-blue-600 to-cyan-600 flex items-center justify-center text-white font-bold text-sm shrink-0">{sub.name.charAt(0).toUpperCase()}</div>
                      <div><p className="font-semibold text-white">{sub.name}</p><p className="text-gray-500 text-xs">{sub.email}</p></div>
                    </div>
                    <div className="flex items-center gap-3 shrink-0">
                      <div className={`text-xs font-bold ${integrityColor}`}>🛡️ {sub.integrityScore}</div>
                      {form.saved && <div className="badge badge-green"><CheckCircle className="w-3 h-3" /> Evaluated</div>}
                      {form.saved && <div className="badge badge-blue font-bold">{total}/40</div>}
                  ...
                  {isOpen && (
                    <div className="border-t border-white/5 p-6 space-y-8 animate-fade-in">
                      <div>
                        <h3 className="font-bold text-white text-sm mb-4 flex items-center gap-2"><span>📋</span> Submitted Answers</h3>
                        <div className="space-y-4">
                          {QUESTION_TITLES.map((title, qi) => {
                            ...
                            return (
                              <div key={qi} className="p-4 rounded-xl bg-white/5 border border-white/5">
                                ...
                                <p className="text-gray-300 text-sm leading-relaxed">{ans?.text || <span className="text-gray-600 italic">No answer submitted</span>}</p>
                              </div>
                            );
                          })}
                        </div>
                      ...
                      <div>
                        <h3 className="font-bold text-white text-sm mb-4 flex items-center gap-2"><Star className="w-4 h-4 text-amber-400" /> Score This Submission</h3>
                          ...
                          {SCORE_CRITERIA.map(({ key, label, desc }) => (
                            <div key={key} className="p-4 rounded-xl bg-white/5 border border-white/5">
                              <div className="flex justify-between items-center mb-1">
                                ...
                                <span className="text-2xl font-extrabold text-blue-400 w-10 text-center">{form.scores[key]}</span>
                              </div>
                              <p className="text-gray-500 text-xs mb-3">{desc}</p>
                              <input id={`score-${sub.uid}-${key}`} type="range" min={0} max={10} step={1} value={form.scores[key]} onChange={(e) => updateScore(sub.uid, key, Number(e.target.value))} className="w-full accent-blue-500" />
                              <div className="flex justify-between text-xs text-gray-600 mt-1"><span>0</span><span>5</span><span>10</span></div>
                            </div>
                          ))}
                        </div>
                        ...
                        <div className="mt-4">
                          <label className="text-sm font-medium text-gray-300 mb-1.5 block">Remarks / Feedback</label>
                          <textarea id={`remarks-${sub.uid}`} value={form.remarks} onChange={(e) => updateRemarks(sub.uid, e.target.value)} placeholder="Optional feedback for the organizer..." className="input-field resize-none" rows={3} />
                        </div>
                        <button id={`save-eval-${idx}`} onClick={() => handleSaveEvaluation(sub)} disabled={form.saving} className="btn-primary w-full mt-4 flex items-center justify-center gap-2">
                          {form.saving ? <><Loader2 className="w-4 h-4 animate-spin" /> Saving...</> : form.saved ? <><CheckCircle className="w-4 h-4" /> Update Evaluation</> : <><Send className="w-4 h-4" /> Save Evaluation</>}
                        </button>
                      </div>
                    </div>
                  )}
